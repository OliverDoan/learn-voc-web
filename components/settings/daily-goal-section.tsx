...
/** Cấu hình số từ học mỗi ngày. Khởi tạo state từ progress (đã có sẵn). */
export function DailyGoalSection({ progress }: DailyGoalSectionProps) {
  const update = useUpdateProgress();
  const [dailyGoal, setDailyGoal] = useState(() => String(progress.dailyGoal));
  ...
  return (
    <section className="rounded-2xl border bg-card p-5">
      ...
      <p className="mb-4 text-sm text-muted-foreground">
        Số từ bạn muốn ôn/học mỗi ngày. Hệ thống sẽ ưu tiên đẩy đủ số này lên hàng
        đợi.
      </p>
